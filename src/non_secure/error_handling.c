...
const retry_config_t RETRY_CONFIG_USB = {.max_attempts = 3,
                                         .base_delay_ms = 100,
                                         .max_delay_ms = 1000,
                                         .exponential_backoff = true};

const retry_config_t RETRY_CONFIG_PROTOCOL = {.max_attempts = 2,
                                              .base_delay_ms = 50,
                                              .max_delay_ms = 200,
                                              .exponential_backoff = false};

const retry_config_t RETRY_CONFIG_CRYPTO = {.max_attempts = 3,
                                            .base_delay_ms = 10,
                                            .max_delay_ms = 100,
                                            .exponential_backoff = true};

const retry_config_t RETRY_CONFIG_STORAGE = {.max_attempts = 5,
                                             .base_delay_ms = 20,
                                             .max_delay_ms = 500,
                                             .exponential_backoff = true};
...
void error_handling_init(void) {
  ...
  printf(
      "Error Handling: Initializing comprehensive error management system\n");
...
void error_report(error_code_t code, error_severity_t severity,
  ...
  printf("ERROR [%s:%s] %s:%d in %s() - Code:0x%04X - %s\n",
         severity_str[severity], category_str[last_error.category], file, line,
         function, code, last_error.message);
...
void usb_stability_update_state(usb_connection_state_t new_state) {
  if (usb_stability.state != new_state) {
    ...
    printf("USB Stability: State change %d -> %d (after %dms)\n",
           ...
           now - usb_stability.last_state_change);
    ...
    if ((now - usb_stability.last_state_change) < 1000) {
      ...
      if (usb_stability.enumeration_failures > 3) {
        ...
        printf("USB Stability: Entering stability mode due to rapid state "
               "changes\n");
      }
    }
...
bool retry_operation(bool (*operation)(void), const retry_config_t *config) {
  return retry_operation_with_context((bool (*)(void *))operation, NULL,
                                      config);
}
...
bool system_is_healthy(void) {
  return (usb_stability_is_stable() && error_count < 50 && !timeout_active);
}
