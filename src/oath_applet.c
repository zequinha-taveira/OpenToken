...
static bool calculate_hmac_sha1(const uint8_t *key, uint8_t key_len, 
    ...
    return (mbedtls_md_hmac(md_info, key, key_len, data, data_len, hmac_out) == 0);
}
...
// Calculate TOTP code
static bool calculate_totp(const storage_oath_entry_t *entry, uint32_t *code_out) {
...
bool oath_applet_select(const uint8_t *aid, uint8_t len) {
  if (len == OATH_AID_LEN && memcmp(aid, OATH_AID, OATH_AID_LEN) == 0) {
    is_selected = true;
    printf("OATH Applet: Selected successfully (YubiKey Manager compatible).\n");
...
#define SET_SW(sw)                                                             \
  ...
  *response_len = 0; // Start with empty response payload
  ...
  case OATH_INS_VALIDATE: {
    // YubiKey Manager sends VALIDATE command to check if OATH applet is available
    // This is used for device detection
  ...
  case OATH_INS_SET_CODE: {
    ...
    printf("OATH Applet: SET_CODE Command - password protection not implemented.\n");
  ...
  case OATH_INS_PUT: {
    ...
    uint8_t key_buf[64];
    uint8_t key_len = 0;
    uint8_t property = OATH_TYPE_TOTP | OATH_HASH_SHA1; // Default: TOTP with SHA1

    // Parse TLV structure
    while (offset < lc_val) {
      if (offset + 1 >= lc_val) break; // Ensure we have at least tag and length
      
      uint8_t tag = data[offset++];
      uint8_t len = data[offset++];
      
      if (offset + len > lc_val) break; // Ensure we don't read beyond data
  ...
  case OATH_INS_LIST: {
    ...
    for (int i = 0; i < STORAGE_OATH_MAX_ACCOUNTS; i++) {
      if (storage_load_oath_account(i, &entry)) {
        ...
        if (*response_len + 2 + total_inner_len > 256) { // Assume max 256 byte response
  ...
  case OATH_INS_CALCULATE: {
    ...
    // Parse TLV structure
    while (offset < lc_val) {
      if (offset + 1 >= lc_val) break;
      
      uint8_t tag = data[offset++];
      uint8_t len = data[offset++];
      
      if (offset + len > lc_val) break;
