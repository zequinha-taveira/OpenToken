...
void ccid_send_status_word(uint16_t sw, uint8_t *out_buffer,
                           uint16_t *out_len) {
...
// APPLET MANAGEMENT
//--------------------------------------------------------------------+
ccid_applet_t ccid_get_selected_applet(void) { return current_applet; }

void ccid_reset_applet_selection(void) { current_applet = APPLET_NONE; }
...
void opentoken_process_ccid_apdu(uint8_t const *buffer, uint16_t len,
  ...
  case APPLET_OATH:
    ...
    if (!retry_operation_with_context(
            (bool (*)(void *))oath_applet_process_apdu_wrapper, &(struct {
              const uint8_t *buf;
              uint16_t len;
              uint8_t *out;
              uint16_t *out_len;
  ...
  case APPLET_OPENPGP:
    ...
    if (!retry_operation_with_context(
            (bool (*)(void *))openpgp_applet_process_apdu_wrapper, &(struct {
              const uint8_t *buf;
              uint16_t len;
              uint8_t *out;
              uint16_t *out_len;
