...
bool ctap2_verify_user_presence(void) {
    ...
    if (!timeout_start(DEFAULT_TIMEOUTS.user_presence_timeout_ms)) {
        ERROR_REPORT_ERROR(ERROR_TIMEOUT_USER_PRESENCE, "Failed to start user presence timeout");
        return false;
    }
...
uint8_t ctap2_generate_credential_id(const uint8_t *rp_id_hash, 
    ...
    // Format: "OT-" + first 8 bytes of RP hash + first 4 bytes of user ID + counter
...
static bool ctap_send_response(uint32_t cid, uint8_t cmd, const uint8_t *data,
    ...
    if (!timeout_start(DEFAULT_TIMEOUTS.usb_operation_timeout_ms)) {
        ERROR_REPORT_ERROR(ERROR_TIMEOUT_USB_OPERATION, "Failed to start USB timeout");
        return false;
    }
    ...
    while (sent < len) {
        ...
        if (!tud_hid_report(0, report, 64)) {
            timeout_reset();
            ERROR_REPORT_ERROR(ERROR_USB_ENDPOINT_ERROR, "Failed to send continuation HID report");
            return false;
        }
        timeout_reset();
...
uint8_t ctap2_handle_make_credential(const uint8_t *cbor_data, uint16_t cbor_len, 
    ...
    uint8_t cred_id[64];
    uint16_t cred_id_len;
    uint8_t result = ctap2_generate_credential_id(rp_id_hash, user_id, user_id_len, 
                                                cred_id, &cred_id_len);
    if (result != CTAP2_OK) {
        return result;
    }
    ...
    uint16_t auth_data_len = ctap_build_authdata(auth_data, rp_id_hash, flags, 0,
...
uint8_t ctap2_handle_get_assertion(const uint8_t *cbor_data, uint16_t cbor_len,
    ...
    uint16_t sig_len;
    if (!hsm_sign_ecc(cred.priv_key, sign_data, auth_data_len + 32, signature, &sig_len)) {
        return CTAP2_ERR_PROCESSING;
    }
...
void opentoken_process_ctap2_command(uint8_t *buffer, uint16_t len) {
    ...
    if (!buffer) {
        ERROR_REPORT_ERROR(ERROR_PROTOCOL_INVALID_COMMAND, "Null buffer in CTAP2 command");
        return;
    }
    ...
    uint32_t cid = buffer[0] | (buffer[1] << 8) | (buffer[2] << 16) | (buffer[3] << 24);
    ...
    if (cmd == (CTAPHID_CMD_INIT | CTAPHID_INIT_FLAG)) {
        ...
        switch (ctap_method) {
            ...
            case CTAP2_GET_ASSERTION:
                status = ctap2_handle_get_assertion(buffer + 8, payload_len - 1,
                                                  response, &response_len);
                break;
