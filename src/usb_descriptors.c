...
//--------------------------------------------------------------------+
enum { ITF_NUM_HID, ITF_NUM_CCID, ITF_NUM_TOTAL };
...
uint8_t const desc_configuration[] = {
    ...
    TUD_HID_DESCRIPTOR(ITF_NUM_HID, 0, HID_ITF_PROTOCOL_NONE,
                       CFG_TUD_HID_REPORT_DESC_LEN, 0x81, 0x01,
                       CFG_TUD_HID_EP_BUFSIZE, 1),

    // 2. Interface CCID (OATH/OpenPGP)
    TUD_CCID_DESCRIPTOR(
        ITF_NUM_CCID, 0, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A,
...
void tud_ccid_xfr_block_cb(uint8_t slot, uint8_t const *buffer,
                           uint16_t bufsize) {
  // Buffer para a resposta APDU (Max 261 bytes + slack)
  ...
  // Envia a resposta APDU de volta ao host
